...
int main()
{
	...
	Shader StencilShader("Vshader.glsl", "StencilFShader.glsl");
...
glm::vec3(-1.3f,  1.0f, -1.5f)
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		Render(window, lightingShader, model, projection, view, dirLightDir, pointLightPositions, spotLightPos, spotLightDir, ourModel);
		...
		model = glm::scale(model, glm::vec3(0.201f, 0.201f, 0.201f));	// it's a bit too big for our scene, so scale it down

		Render(window, StencilShader, model, projection, view, dirLightDir, pointLightPositions, spotLightPos, spotLightDir, ourModel);
